...
function fn_detailAttrEnabled_${reqParam.changeFlag}(param){
    ...
    $(formDetailDiv).find(".dailyTr").each(function(idx){
        ...
        }else if(0<idx && idx<$(formDetailDiv).find(".dailyTr").length - 1){
            $(this).find("[class*='sHM']").prop("disabled", true)
        }
    })
}
...
$("#formDetailDiv_${reqParam.changeFlag} [class*='sBEZ']").change(function(){
    ...
    if($(dailyTr).index() > 0 || sBEZ_C =="" || sBEZ_M==""){
		return ;
	}
    ...
    var restTime = getRestTime(sBEZ_C, sBEZ_M, sENZ_C, sENZ_M, param.zlcty); 
    sENZ = Number(sBEZ) + Number(max) + restTime;
    sENZ_C = String(sENZ).split('.')[0];
    sENZ_C = sENZ_C > 10 ? sENZ_C : fillZero(2,sENZ_C);
	sENZ_M = String(sENZ).split('.')[1] == '5' ? '30': '00';
	
    $(this).find("[class*='sENZ'] option").prop("selected",false);

	if(Number(sENZ) > 19){
        alert("종료시간은 19시 이전 이어야 합니다");
	}
    if(Number(sENZ) > 19.5 ){
        $("#formDetailDiv_after .dailyTr").each(function(idx){
        	if(idx > 0){
	        	$(this).find("[class*='sBEZ'] option[value='']").prop("selected",true)
        	}
        	$(this).find("[class*='sENZ'] option[value='']").prop("selected",true)
            $(this).find(".oARS").val('');
        })
    }else{
	    $("#formDetailDiv_after .dailyTr").each(function(idx){
	    	if(idx == $("#formDetailDiv_after .dailyTr").length-1){
	    		return false;
	    	}
		    $(this).find(".sBEZ_C option[value='"+sBEZ_C+"']").prop("selected",true)
		    $(this).find(".sBEZ_M option[value='"+sBEZ_M+"']").prop("selected",true)
		    $(this).find(".sENZ_C option[value='"+sENZ_C+"']").prop("selected",true)
		    $(this).find(".sENZ_M option[value='"+sENZ_M+"']").prop("selected",true)
	        
		    var workTime = calcWorkTime(sBEZ_C, sBEZ_M, sENZ_C, sENZ_M) ;
		    $(this).find(".oARS").val(workTime);
	    })
    }
...
// 시간 선택 
$("#formDetailDiv_${reqParam.changeFlag} .dailyTr:eq(4) .sHM").change(function(){
    var dailyTr = $(this).parents(".dailyTr");
    $(dailyTr).find(".oARS").val("");
    ...
    var min = $("#sTMDTY_after option:selected").attr("min").trim();
    if(workTime != min){
        alert("근무시간은 "+min+"시간이 되어야합니다")
    }
})
...
function saveChk_byType(){
    ...
    var alertMsg = "";
   	...
   	var min = $("#sTMDTY_after option:selected").attr("min").trim();
    $("#formDetailDiv_after .dailyTr").each(function(idx) {
    	var sENZ_C = nvl($(this).find(".sENZ_C option:selected").val(), "");
    	var sENZ_M = nvl($(this).find(".sENZ_M option:selected").val(), "");
    	
    	if(Number("".concat(sENZ_C, sENZ_M)) > 1900){
    		alertMsg = "종료시간은 19시이전 이여야 합니다";
            flag = true;
            return false;
    	}
    	
    	if(idx == $("#formDetailDiv_after .dailyTr").length-1){
    		if($(this).find(".oARS").val() != min){
	    		alertMsg = "금요일의 근무시간은 "+min+"시간이어야 합니다";
	            flag = true;
	            return false;
    		}
    	}else{
    		console.log($(this).find(".oARS").val())
    		if($(this).find(".oARS").val() != max){
	    		alertMsg = "월,화,수,목요일의 근무시간은 "+max+"시간이어야 합니다";
	            flag = true;
	            return false;
    		}
