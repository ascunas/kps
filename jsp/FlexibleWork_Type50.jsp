...
function saveChk_byType(){
    ...
    var sENZ_C = nvl($("#timeSettingTable").find(".sENZ_C option:selected").val(), "");
    var sENZ_M = nvl($("#timeSettingTable").find(".sENZ_M option:selected").val(), "");
    var sFAMTY = $("#formDetailDiv_after select[name=sFAMTY] option:selected").val();
...
<%--                                     <input type="text" name="${ENDDA_NM}" id="${ENDDA_NM}" value="${oENDDA_Comm}"/><label for="${ENDDA_NM}" onclick="$.showCal2('${ENDDA_NM}')" style="vertical-align:top;">달력</label> --%>
        ...
        <c:otherwise>
            <c:if test="${reqParam.ACTION eq 'APPLY' || (reqParam.ACTION eq 'UPDATE' && reqParam.changeFlag eq 'after')}">
                ...
                <tbody>
                    ...
                    <tr class="dailyTr">
                        ...
                        <td style="text-align:center; position: relative;">
                            ...
                            <span style="top:10px; width:250px; position: absolute;">
                                <span style="margin-right: 20px;">자녀선택</span>
						        <select name="sFAMTY" class="select sFAMTY sForm"></select>
						    </span>
                            </c:if>
                        </td>
                    </tr>
